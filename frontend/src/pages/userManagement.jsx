...
function UserManagement() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/users") // Gọi API để lấy danh sách người dùng
      .then((response) => {
  ...
  const handleInputChange = (e, userId, field) => {
    ...
    setChangedUsers((prev) => ({
      ...prev,
      [userId]: { ...updatedUsers.find((user) => user.id === userId) },
    }));
  };
  ...
  const handleSaveAll = () => {
    const savePromises = Object.keys(changedUsers).map((userId) => {
      return axios
        .put(`http://localhost:3001/users/${userId}`, changedUsers[userId])
        .then((response) => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:3001/users/${id}`) // Gọi API để xóa người dùng
      .then(() => {
          ...
          <tbody>
            ...
            {users.length > 0 ? (
              users.map((user) => (
                <tr
                  key={user.id}
                  className="border-b hover:bg-orange-100 bg-pink-100"
                >
                  <td className="p-3 px-5">
                    <input
                      ...
                      value={user.username}
                      onChange={(e) =>
                        handleInputChange(e, user.id, "username")
                      }
